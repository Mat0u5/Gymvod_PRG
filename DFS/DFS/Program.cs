    ...
    internal class Program
    {
        ...
        static List<int> DepthFirstSearch(BinaryTree graph, int targetNodeIndex)
        {
            ...
            while (currentNode.index != targetNodeIndex)
            {
                ...
                bool returnUp = true;
                //Console.WriteLine($"parent {currentNode.index}");
                foreach (Node child in children)
                {
                    if (child == null) continue;
                    hasAnyChildren = true;
                    if (!currentNode.visitedChildren.Contains(child))
                    {
                        //Console.WriteLine($"child {child.index}");
                        currentNode.visitedChildren.Add(child);
                        pathToTargetNode.Add(child.index);
                        currentNode = child;
                        returnUp = false;
                        break;
                    }
                }
                if (!hasAnyChildren || returnUp)
                {
                    pathToTargetNode.Remove(currentNode.index);
